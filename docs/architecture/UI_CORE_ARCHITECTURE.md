...
*   **Responsibilities:**
    *   No longer performs local downsampling. Delegates this responsibility entirely to the Core (`IWorkingImageHardware::downsample`). Manages zoom and pan state. Sends display-ready images (received from `ImageControllerBase`) to the appropriate Qt Quick rendering item (`RHIImageItem`, `SGSImageItem`, etc.). Notifies the Core layer of the source image size via `setSourceImageSize`.
    *   **ViewportManager Integration:** Integrates `ViewportManager` (`m_viewport_manager`). The initialize method configures the `ViewportManager` with screen and viewport details. The `setSourceImageSize` method uses `m_viewport_manager->calculateDisplay(...)` to determine the required downsampled size, emitting `displayImageRequest` if downsampling is needed. The `downsampleSize` and `maxDownsample` are exposed as QML properties. The `fitToView` and `setViewportSize` methods utilize calculations from the `ViewportManager`. A new method `setQualityMargin` is added to control the quality vs. performance trade-off via the `ViewportManager`.
    *   Logical Coordinate Space Management: `fitToView()` and `constrainPan()` calculate zoom and pan offsets strictly relative to `m_display_image_size` (the QML item's local dimensions), rather than the global window viewport. This is critical because the QML item sizes itself to `displayImageSize` and uses `anchors.centerIn: parent` to handle the global centering. Calculating pan against the global viewport causes a double-offset bug, shifting portrait images off-center.
...
*   **Responsibilities:**
    ...
    *   `RHIImageItem`:  Uses `QQuickRhiItem` to leverage `QRhi` (Vulkan, Metal, DirectX12). Integrates `RHIImageItemRenderer` for direct manipulation of the render pipeline.
    *   - **Memory Management & Signatures:** Signatures updated for `std::unique_ptr`. Implements `onZoomChanged`, `onPanChanged`, `onImageChanged` to emit signals and trigger updates. `RHIImageItemRenderer` is declared as a friend.
    *   - **High-DPI (DPR) Matrix Handling:** Because `QQuickRhiItem` operates at a lower level than the Scene Graph, the renderer does not automatically handle logical-to-physical pixel scaling for custom matrices. To prevent the image from rendering in the top-left corner and being cropped on High-DPI screens, the `RHIImageItemRenderer` must retrieve the item's logical dimensions during `synchronize()` (stored as `m_item_width`, `m_item_height`) and use them to build the orthographic projection matrix and centering offsets. It must mirror the exact mathematical logic used by `SGSImageItem` (`x_pos = (logical_w - display_w) / 2.0f + pan.x()`). Using physical render target sizes (`renderTarget()->pixelSize()`) for transformation math is strictly prohibited as it breaks DPR alignment.
    *   `SGSImageItem`: Uses  `QQuickItem ` and  `QSGSimpleTextureNode ` via the Scene Graph.
    *    - **Color Space Handling:** When updating the paint node, the internal linear F32 image data (from `m_full_image`) is interpreted using `QImage::Format_RGBA32FPx4`, assigned the `QColorSpace::SRgbLinear` color space, and then converted to `QImage::Format_RGBA8888` with the standard `QColorSpace::SRgb` for display, leveraging Qt's color space conversion utilities.
    *    - **Coordinate Space Handling:** Implements zoom and pan positioning by calculating the final geometry (`setRect`) strictly in logical pixels (using the item's `width()` and `height()`), seamlessly delegating High-DPI (DPR) scaling to Qt's Scene Graph abstraction.
