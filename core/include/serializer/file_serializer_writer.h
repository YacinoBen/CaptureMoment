...
public:
     ...
     *                          Must not be null.
     */
    explicit FileSerializerWriter(std::unique_ptr<IXmpProvider> xmp_provider, std::unique_ptr<IXmpPathStrategy> xmp_path_strategy);
    ~FileSerializerWriter() override = default;

    // Delete copy/move for simplicity, assuming unique_ptr dependencies
     ...
     */
    [[nodiscard]] bool saveToFile(std::string_view source_image_path, std::span<const Operations::OperationDescriptor> operations) const override;
